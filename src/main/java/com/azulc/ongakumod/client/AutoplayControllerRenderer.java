...
{    
    ...
    public void render(AutoplayControllerBlockEntity controller, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) 
    {
        ...
        int statusFrameOutput = switch (statusFrame) {
            case -1 -> 0; // No Jukebox
            case 0 -> 1; // Idle Not playing
            case 1 -> 2; // track is loaded but not playing
        ...
        // Render Status
        renderFace(poseStack, bufferSource.getBuffer(RenderType.entityCutout(OngakuModClient.BOX_STATUS)), 
            5/16f, 12/16f, 7/16f, 14/16f, zFront, statusFrameOutput, 4, packedLight, packedOverlay);
        // Render Progress
        renderFace(poseStack, bufferSource.getBuffer(RenderType.entityCutout(OngakuModClient.BOX_PROGRESS)), 
